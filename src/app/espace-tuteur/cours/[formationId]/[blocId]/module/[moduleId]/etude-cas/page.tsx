...
export default function TutorEtudeCasPage({ params }: EtudeCasPageProps) {
    ...
    const loadModuleInfo = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setModuleInfo(data.module || data.cours);
        }
  ...
  const handleBack = () => {
    router.push(`/espace-tuteur/cours/${formationId}/${blocId}`);
  };
    ...
    <div className="min-h-screen bg-[#F8F5E4]">
      <div className="bg-[#F8F5E4] border-b border-[#032622]/20 px-6 py-4">
        <div className="flex items-center gap-4">
          <button
            onClick={handleBack}
            className="flex items-center justify-center w-10 h-10 bg-[#032622] text-[#F8F5E4] hover:bg-[#032622]/80 transition-colors"
