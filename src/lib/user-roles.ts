...
export type UserRole = 'lead' | 'candidat' | 'etudiant' | 'tuteur' | 'pedagogie' | 'commercial' | 'adv' | 'formateur' | 'admin' | 'superadmin';
...
// Fonction pour vérifier si l'utilisateur est tuteur
export function isTuteur(userRole: UserRole | null): boolean {
  return userRole === 'tuteur';
}
...
export function canAccessRoute(userRole: UserRole | null, route: string): boolean {
  const routePermissions: Record<string, UserRole[]> = {
    ...
    '/espace-tuteur': ['tuteur', 'admin', 'superadmin'],
    '/espace-etudiant': ['etudiant', 'admin', 'superadmin'],
    '/validation': ['lead', 'candidat']
  };
